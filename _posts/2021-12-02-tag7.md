...
Im Naturkunde Museum, war ein Dozent am arbeiten. Die Sammlung war da noch in Exceltabellen. Mit Excel geht das schon, solange begrenzte Anzahl von Datensätzen besteht. Jetzt wollen sie das gerne zusammenführen, wie kriegt man diese unstrukturierten Daten aus dem Excel in ein Format wie dublin core oder marc21?
...
Abgleich mit Normdaten (Renconciliation) in Wikidata, GND und VIAF. (siehe Aufgabe weiter unten)
...
JSOn ist mit etwas Übung einfach zu modellieren.**JSON-API's**:Normalerweise Nutzung von JSON-APIs, wenn nicht im Archiv oder Bibliothekswesen. JSON ist neuer als Marc XML.Ich muss zuerst mal googlen, was jetzt mit JSON-API genau gemeint ist:[json-api.org] (www.jsonapi.org/) https://jsonapi.org/
...
Open refine sei gut für Massendaten, darum hat Terry Reese dieses Tool auch eingesetzt.
...
Darum braucht es einen Abgleich mit Normdaten (=Renconciliation) in Wikidata, GND und VIAF.
...
Vorteil on open refine ist  eben grosse Datenmengen anzeigen kann, Excel ist da schwieriger im Excel gab es früher 246 Spalten und 65'000 Zeilen, gilt zwar heute nicht mehr, war aber noch im Jahr 2013 der Fall.
...
Jetzt werden die Autoren, wo vorher gesplitten waren, wieder zusammengefügt die Autoren, man soll sich Zahl 1001 merken von vorher, jetzt auch wieder 1001 sind! Wenn das nicht der Fall ist, war man in einer falschen Facette!
...
Wikidata gibt es Eintrag von Journals, zuerst müssen wir aber eine Spalte Journals im Open Refine hinzufügen (ähnlich wie Excel, dort kann man auch Spalten hinzufügen):
...
Datensätze anschauen, denn nicht alle Datensäzte sind diesen journals eingeordet,dann soll man lieber allgemeine kategorie drin lassen.
...
(Gibt noch 2. ISSN für die Printausgabe des Journals.)
...
Ich habe auch das Feld mit der Doi gefunden. Diese Datei habe ich dann wieder abgespeichert, aber noch nicht validiert. Ich müsste die übung nochmals machen, diese wurde nur kurz vorgezeigt. Diese wurde auch wieder im Export Templeting gemacht.
...
**Grell Sprache -schwere Sprache** 
Die Grell Sprache, so wie ich es verstanden habe:
...
Oder man kann Nonblade machen, das heisst wenns blank (leer) ist, nichts reinschreiben.
  _Beispiel mit der DOI_
 - Wenns blank ist, dann nicht, und sonst datafield schreiben
 - Wenn Datensätze leer sind, dann nichts und sonst doi.
 - Wenn das Feld nicht leer ist, dann doi schreiben in Feld "024" + Unterfeld "a" "2"=(Doi).
...
**AHA-Moment: So kann man prüfen ob es dem Schema marc21 entspricht, hatten wir im Modul "Gset" sonst immer mit einem Validierungs-Tool im Browser gemacht, das war neu für mich und dann erfolgt die Validierung.**
